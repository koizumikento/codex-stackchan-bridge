...
SERVICE_FIELDS = {
    ...
    "NextAudioChunk.srv": [
        ...
        "bool should_publish_window",
        "uint32 publish_from_sequence",
        "uint32 publish_window_chunks",
    ],
    "LoadAudioChunk.srv": [
        "stackchan_msgs/CommandMeta meta",
        "uint32 sequence",
...
def main() -> int:
    ...
    for action in (MSGS / "action").glob("*.action"):
        ...
        if action.name == "Say.action":
            require("string<=32 face_hint" in text, "Say missing face hint")
            require("string<=32 motion_hint" in text, "Say missing motion hint")
            require("string<=32 after_face_hint" in text, "Say missing after-face hint")
